...
#' 
#' ADAPTED FROM SUMMER getSmoothed TO RETURN NMR https://github.com/richardli/SUMMER/blob/master/R/projINLA.R
#'
...
#' @export
custom_getSmoothed <- function(inla_mod, nsim = 1000, weight.strata = NULL, weight.frame = NULL, verbose = FALSE, mc = 0, include_time_unstruct = FALSE, CI = 0.95, draws = NULL, save.draws = FALSE, include_subnational = TRUE, ...){
  ...
  if(!is.null(inla_mod$family)){
    ...
    # CUSTOM LINE TO RETURN NMR.
    # The product on line 602 of original code becomes (1-1q0)^1 * (1 - 12q1)^0 * (1 - 24q12)^0 * …
    age.nn[age.nn != 1] <- 0
    # if(!is.dynamic){
